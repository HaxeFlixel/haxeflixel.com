...
function runBuildImage () {
  removeBuildImage()
  const containerID = execSync(`docker run --name="${BUILD_CONTAINER_NAME}" -dti ${IMAGE_NAME} bash`)
...
function watchSite () {
  ...
  const containerID = execSync(`docker run -p 9778 --name="${WATCH_CONTAINER_NAME}" -dti ${IMAGE_NAME} bash`)
  const ip = execSync(`docker inspect --format '{{ .NetworkSettings.IPAddress }}' ${containerID}`)
